...
def checkParkingSpace(imgPro):
    ...
    # Check if free spots have changed
    if spaceCounter != prev_free_spots:
        prev_free_spots = spaceCounter
        occupied_spaces = len(posList) - spaceCounter
        print(f"Total Spaces: {len(posList)}, Free: {spaceCounter}, Occupied: {occupied_spaces}")
        return occupied_spaces  # Return when there's a change
